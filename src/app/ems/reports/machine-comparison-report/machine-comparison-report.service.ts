...
export class MachineComparisonReportService {
  ...
  getMachineCompareData = data => {
    ...
    let tempData = {};
    let machine_names: string[] = [];

    tempData['start_time'] = this.formatDate(data.from);
    tempData['end_time'] = this.formatDate(data.to);

    // let tempData = Object.assign({}, data); machineName

    if (data.firstMachineId !== '') {
      machine_names.push(data.firstMachineId.assosiativeName);
    }
    if (data.secondMachineId !== '') {
      machine_names.push(data.secondMachineId.assosiativeName);
    }
    if (data.thirdMachineId !== '') {
      machine_names.push(data.thirdMachineId.assosiativeName);
    }
    if (data.fourthMachineId !== '') {
      machine_names.push(data.fourthMachineId.assosiativeName);
    }
  ...
  getPlantWiseMachine = (plantId, machineType, discardedMachines) => this.rest.post
    (`config/machine/filter/comparisionMachine`, { plantId, machineType, discardedMachines });
